...
import { PermissionEngine, type PermissionConfig, type PermissionDecision } from "./permission.js";
...
export class Agent {
  ...
  constructor(opts: AgentOptions) {
    ...
    this.parallelInputsStable =
      ...
      !Boolean(opts.permission?.confirm && (opts.permission.askRules?.length ?? 0) > 0);
  }
  ...
  async *send(userText: string, signal?: AbortSignal): AsyncGenerator<AgentEvent> {
    if (this.running) throw new Error("会话正忙：上一轮尚未结束（运行中追加输入请用 queue）");
    this.running = true;
  ...
  private async prepareUserInput(
    ...
    return { blocked: false, ...(h.additionalContext ? { additionalContext: h.additionalContext } : {}) };
  }
  ...
  /** 并发资格按调用判定；只允许输入稳定、声明只读且未显式禁并发的工具。 */
  private isParallelSafe(call: ToolCall): boolean {
    const tool = this.tools.get(call.name);
    // 只允许契约上无副作用且入参不会在准备阶段被改写的工具进入并发批。
    if (!tool?.readOnly || !this.parallelInputsStable) return false;
  ...
  private async *runToolBatch(
    ...
    const runs = batch.map(async (call, idx) => {
      ...
      try {
        ...
        chan.push({ type: "tool_result", id: call.id, name: call.name, content: msg, isError: true });
      }
      if (!local[0]) {
        const msg = "工具未返回结果";
        local.push(errResult(call.id, call.name, msg));
  ...
  private async *streamText(
    ...
    try {
      ...
      for await (const ev of this.provider.stream({ model: this.model, system, messages, maxTokens })) {
        if (ev.type === "text_delta") yield { type: "text", text: ev.text };
      }
    }
  }
