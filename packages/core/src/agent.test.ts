...
test("Agent: 工具调用 → 执行 → 结果回传 → 收尾", async () => {
  ...
  const provider = scriptedProvider([
    ...
    [{ role: "assistant", content: [
      ...
      { type: "tool_call", id: "c1", name: "write", args: { path: "hello.txt", content: "hi" } },
    ] }],
    // 第二轮：读回来
    [{ role: "assistant", content: [
      { type: "tool_call", id: "c2", name: "read", args: { path: "hello.txt" } },
    ] }],
    // 第三轮：收尾
...
test("Agent: 权限拒绝 → 错误结果回传给模型", async () => {
  ...
  const agent = new Agent({
    ...
    model: "x",
    cwd: dir,
    permission: { mode: "default", confirm: async () => ({ behavior: "deny", message: "危险命令被拒绝" }) },
  });
...
test("Agent: resume 悬空 tool_call 自愈 + 补写落盘", async () => {
  ...
  await store.append("s_crash", { role: "assistant", content: [
    { type: "tool_call", id: "c9", name: "bash", args: { command: "ls" } },
  ] });
  ...
  const provider: Provider = {
    ...
    async *stream(req: any): AsyncIterable<StreamEvent> {
      ...
      sawRepairedHistory =
        last.role === "user" &&
        last.content.some((p: any) => p.type === "tool_result" && p.toolCallId === "c9" && p.isError);
      yield {
        type: "done",
        stopReason: "end_turn",
...
test("Agent: 只读工具在 default 模式下自动放行（不触发 confirm）", async () => {
  ...
  const provider = scriptedProvider([
    [{ role: "assistant", content: [
      { type: "tool_call", id: "c1", name: "read", args: { path: "a.txt" } },
    ] }],
  ...
  const agent = new Agent({
    ...
    model: "x",
    cwd: dir,
    permission: { mode: "default", confirm: async () => { confirmCalled = true; return { behavior: "deny" }; } },
  });
