...
test("Agent features: steering 同样经过 UserPromptSubmit hook，内部 context 不冒充用户原话", async () => {
  ...
  assert.ok(
    second.content.some((p) => p.type === "text" && p.internal && /secret-policy/.test(p.text)),
  );
...
test("Agent features: 部分流失败重试前发 turn_reset，历史只保留成功尝试", async () => {
  ...
  assert.deepEqual(
    events
      .filter((e) => e.type === "text" || e.type === "turn_reset" || e.type === "retry")
      .map((e) => e.type),
...
test("Agent features: 部分流最终失败也发 turn_reset，不把残影留在 UI", async () => {
  ...
  assert.deepEqual(
    events
      .filter(
        (event) => event.type === "text" || event.type === "turn_reset" || event.type === "error",
      )
      .map((event) => event.type),
    ["text", "turn_reset", "error"],
  );
  assert.equal(
    agent.messages.some((m) => m.role === "assistant"),
    false,
  );
...
test("Agent features: confirm 改写后的最终动作仍受 deny 规则约束", async () => {
  ...
  assert.equal(runs, 0);
  assert.ok(events.some((event) => event.type === "tool_permission" && event.decision === "deny"));
...
test("Agent features: PostToolUse 可观察错误，PreToolUse context 不丢失", async () => {
  ...
  const failing: Tool = {
    ...
    ruleKey: () => "fail",
    async run() {
      throw new ToolError("tool failed");
    },
  };
  let sawError = false;
  const provider = scriptedProvider([
...
test("SessionManager: steering send Promise 等到输入已处理且持久化后才完成", async () => {
  ...
  const steering = manager.send(session.id, "B").then(() => {
    steeringResolved = true;
  });
...
test("SessionManager: interrupt 后同 tick 的新 send 进入下一 drive", async () => {
  ...
  assert.ok(
    secondCallUsers.some((message) =>
      message.content.some((part) => part.type === "text" && part.text === "B"),
    ),
  );
  const shownB = events.find(
    (event) =>
