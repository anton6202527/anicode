...
test("overlay: 命令菜单超长列表开窗滚动（不显翻页提示）", () => {
  ...
  // 高度封顶：不铺满 24 行整屏。
  assert.ok(s.lines.length < 24, `菜单高度未封顶：${s.lines.length}`);
});

test("overlay: 模型选择器高度封顶不铺满整屏", () => {
  ...
  const s = buildModelPickerOverlay(big, 0, "", 30, 80);
  assert.ok(s.lines.length <= Math.floor(30 * 0.7), `弹框铺满整屏：${s.lines.length}/30`);
  const text = s.lines.map(strip).join("\n");
